...
$id = $_GET['id'];
$author = mysqli_query($koneksi, "SELECT * FROM user_dindafazryan WHERE id = '$id'");
$penulis = mysqli_fetch_array($author);

$postinganByPenulis = mysqli_query($koneksi, "SELECT * FROM 
...
$jmlData = mysqli_num_rows($postinganByPenulis);
$totalHalaman = $jmlData / $batas;
$nomor = $halamanAwal + 1;

$postinganPaging = mysqli_query($koneksi, "SELECT * FROM 
postingan_dindafazryan 
LEFT JOIN kategori_dindafazryan 
ON postingan_dindafazryan.id_kategori = kategori_dindafazryan.id
LEFT JOIN user_dindafazryan
ON postingan_dindafazryan.id_user = user_dindafazryan.id
WHERE id_user = '$id' ORDER BY id_postingan DESC 
...
<head>
    ...
    <title><?= $penulis["username"] ?>, Penulis di Fazzblog</title>
</head>
                ...
                <div class="col-lg-12">
                    <div class="row mb-2">
                        <div class="col-12 col-md-7 order-1 order-md-0">
                            <h3>Menampilkan postingan <span class="fw-bold"> <?= $penulis["username"] ?>.</span></h3>
                        </div>
                        <div class=" col-12 col-md-5">
                            <form action="" method="get">
                                <div class="d-flex mb-3">
                                    <input type="text" class="f-form me-2" placeholder="Cari artikel di Fazzblog">
                                    <button class="m-btn pri-btn" type="button" name="cariArtikel">
